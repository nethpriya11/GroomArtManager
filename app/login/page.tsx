...
export default function LoginPage() {
  ...
  async function handleManagerLogin(data: LoginInput) {
    setLoading(true)
    try {
      const user = await signInWithCredentials('manager@salonflow.com', data.password)
   ...
   */
  async function handleBarberClick() {
    setLoading(true)
    try {
      const user = await signInAsHardcodedBarber()
      login(user)
      toast.success('Welcome Barber!')
      router.push('/barber/dashboard')
    } catch (error) {
      handleError(error, 'LoginPage.handleBarberClick')
      toast.error('Failed to log in as Barber.')
    } finally {
      setLoading(false)
    }
  }
